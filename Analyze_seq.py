...
def ard2(file, pattern) :
	...
	for p in proteoms :
		...
		with open(p, "r") as filin :
			...
			elif basename(p) == 'New_Proteom_All.txt' : 	
				for line in filin :
					if line.startswith('>') :
						idt = line.strip()
						dico_porph[idt] = ""
					else :
						dico_porph[idt] += line.strip()
			#elif basename(p) == 'New_Proteom_Proteom_All.txt' : 	
			#	for line in filin :
			#		if line.startswith('>') :
			#			idt = line.strip()
			#			dico_str(name)[idt] = ""
			#		else :
			#			dico_str(name)[idt] += line.strip()
			'''
			else : 	
				for line in filin :
					if line.startswith('>') :
	...
	elif basename(file) == 'STDOUT_porph' :
		for idt in dico_porph.keys() :
			dico_f[idt] = {}
